...
{
    public bool ApplyForceOnce;
    //Use to switch between Force Modes
    enum ModeSwitching { Start, Impulse, Acceleration, Force, VelocityChange, None };
    ...
    void Update()
    {
        ...
        if (m_ModeSwitching != ModeSwitching.Start)
        {
            ...
            m_NewForce = new Vector3(m_ForceX, 0, m_ForceY);
        }
        ...
        switch (m_ModeSwitching)
        {
            ...
            case ModeSwitching.Acceleration:
                ...
                if(ApplyForceOnce)
                    m_ModeSwitching = ModeSwitching.None;
                break;
